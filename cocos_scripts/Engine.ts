...
export class Engine {
    public static getOccupiedCells(item: ItemInstance, def: ItemDef) {
    ...
    public static checkOverlap(cells1: { x: number, y: number }[], cells2: { x: number, y: number }[]) {
    ...
    public static isAdjacent(cells1: { x: number, y: number }[], cells2: { x: number, y: number }[]) {
        for (const c1 of cells1) {
            for (const c2 of cells2) {
    ...
    public static isDiagonal(cells1: { x: number, y: number }[], cells2: { x: number, y: number }[]) {
        for (const c1 of cells1) {
            for (const c2 of cells2) {
                const dx = Math.abs(c1.x - c2.x);
                const dy = Math.abs(c1.y - c2.y);
    ...
    public static calculateStats(backpack: ItemInstance[]): { stats: PlayerStats, activeSynergies: string[], hasMagic: boolean } {
        ...
        for (let i = 0; i < backpack.length; i++) {
            ...
            const cells1 = this.getOccupiedCells(item1, def1);
            ...
            for (const synergy of def1.synergies) {
                ...
                for (let j = 0; j < backpack.length; j++) {
                    ...
                    if (def2.tags.includes(synergy.targetTag)) {
                        const cells2 = this.getOccupiedCells(item2, def2);
                        if (synergy.direction === 'adjacent' && this.isAdjacent(cells1, cells2)) {
                            synergyActive = true;
                            break;
                        } else if (synergy.direction === 'diagonal' && this.isDiagonal(cells1, cells2)) {
                            synergyActive = true;
                            break;
